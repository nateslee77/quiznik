...
export function StudyDeck({ cards }: { cards: Card[] }) {
  ...
  return (
    <div className="flex flex-1 flex-col items-center">
      <div className="mb-3 flex w-full items-center justify-between text-sm text-amber-950/50">
        <span>{progress}</span>
        <button
      ...
      <button
        ...
        className="my-8 flex aspect-[4/3] w-full max-w-md cursor-pointer items-center justify-center rounded-2xl border border-amber-900/10 bg-white p-8 text-center shadow-sm transition hover:shadow-md"
      ...
      <div className="flex w-full max-w-md items-center gap-3">
        <button
          onClick={prev}
          disabled={index === 0}
          className="flex-1 rounded-lg border border-amber-900/20 px-4 py-3 text-sm font-medium text-amber-950/80 transition hover:bg-orange-100/70 disabled:opacity-40"
        >
          ← Prev
        </button>
        {atEnd ? (
